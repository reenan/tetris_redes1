...
//import fuck from  "./blockrain.jquery.libs.js";
import BlockRain from "./blockrain.jquery.src.js";
import "./blockrain.jquery.themes.js";

((function ( jQuery ) {

  "use strict";

  jQuery.widget('aerolab.tetrify', {
    ...
    start: function() {
      if (!this.options.useEnemyData) {
        ...
        // não deixar as merdas aparecerem..
        this._$start.fadeOut(0);
        this._$gameover.fadeOut(0);
        this._$score.fadeOut(0);
      }
    },
    ...
    _doStart: function() {
      ...
      //console.log("this._$start: ", this._$start)
    ...
    theme: function(newTheme){
      ...
      if( typeof newTheme === 'string' ) {
        ...
        this._theme = jQuery.extend(true, {}, BlockrainThemes[newTheme]);
      }
      else {
      ...
      if( typeof this._theme === 'undefined' || this._theme === null ) {
        this._theme = jQuery.extend(true, {}, BlockrainThemes['retro']);
    ...
    getPieces: function (num) {
     ...
     addConcreteLine: function (lines) {
      let filledData = this._filled.data.splice(0);
      filledData.splice(
        0, // remove da posição 0 até..
        this._BLOCK_WIDTH * lines
      );

      const arrayToConcat = this.getPieces((this._BLOCK_WIDTH * 2) * lines);

      this._filled.data = filledData.concat(arrayToConcat);
      ...
      function Shape(game, orientations, symmetrical, blockType) {
        ...
        jQuery.extend(this, {
          ...
          getBounds: function(_blocks) { // _blocks can be an array of blocks, an orientation index, or undefined
            var blocks = jQuery.isArray(_blocks) ? _blocks : this.getBlocks(_blocks),
      ...
      this._filled = {
        ...
        add: function(x, y, blockType, blockVariation, blockIndex, blockOrientation) {
          if (x >= 0 && x < game._BLOCK_WIDTH && y >= 0 && y < game._BLOCK_HEIGHT) {
            //console.log('this.data[this.asIndex(x, y)]: ', this.data[this.asIndex(x, y)]);
            const square = this.data[this.asIndex(x, y)];
            if (typeof square != 'undefined' && square != null && square.blockType == 'troll') {
              //console.log('ia dar merda! blockType: ', blockType);
              return;
            }
        ...
        _popRow: function(row_to_pop) {
          for (var i=game._BLOCK_WIDTH*(row_to_pop+1) - 1; i>=0; i--) {
            /*if (this.data[i] != null && typeof this.data[i] !== 'undefined' && this.data[i].blockType != 'troll') {
              if (this.data[i-game._BLOCK_WIDTH] == null || this.data[i-game._BLOCK_WIDTH] === 'undefined' || this.data[i-game._BLOCK_WIDTH].blockType != 'troll') {*/
                /*console.log(
                  'this.data[i]: ', (!this.data[i] ? this.data[i] : this.data[i].blockType),
                  'replace: ', (!this.data[i-game._BLOCK_WIDTH] ? this.data[i-game._BLOCK_WIDTH] : this.data[i-game._BLOCK_WIDTH].blockType)
                );*/
        ...
        checkForClears: function() {
          ...
          for (i=0, len=this.data.length; i<len; i++) {
            mod = this.asX(i);
            if (mod == 0) count = 0;
            if (this.data[i] && this.data[i] != null && typeof this.data[i] !== 'undefined' && typeof this.data[i].blockType === 'string' && this.data[i].blockType == 'troll') {
              break;
            }
        ...
        draw: function() {
          for (var i=0, len=this.data.length, row, color; i<len; i++) {
            if (this.data[i] !== undefined && this.data[i] != null /*null pq tá vindo assim do socket.. cara.. sei la são 7 da manhã, convenhamos*/) {
              row = this.asY(i);
      ...
      this._board = {
        ...
        nextShape: function(_set_next_only) {
          ...
          if( typeof game._theme.complexBlocks !== 'undefined' ) {
            if( jQuery.isArray(game._theme.complexBlocks[result.blockType]) ) {
            ...
            if( jQuery.isArray(game._theme.blocks[result.blockType]) ) {
            ...
            if (drop) {              
              ...
              if (game._checkCollisions(x, y+1, blocks, true)) {
                ...
                for (var i=0; i<cur.blocksLen; i+=2) {
                  ...
                  if (y + blocks[i] < 0) {// its a bug if you drop only shape "line"
                    gameOver = true;
                  }
                  blockIndex++;
                }
                ...
                game.options.onPlaced.call(game.element, game._filled.data);
              }
            }
          }
    ...
    _preloadThemeAssets: function() {
      ...
      var startAssetLoad = function(block) {
        ...
        if( jQuery.isArray(block) && block.length > 0 ) {
    ...
    _createHolder: function() {
      ...
      this._$gameholder = jQuery('<div class="blockrain-game-holder"></div>');
      ...
      this._$canvas = jQuery('<canvas style="display:block; width:100%; height:100%; padding:0; margin:0; border:none;" />');
      if( typeof this._theme.background === 'string' ) {
        this._$canvas.css('background-color', this._theme.background);
      }
    ...
    _createControls: function() {
      ...
      game._$touchLeft = jQuery('<a class="blockrain-touch blockrain-touch-left" />').appendTo(game._$gameholder);
      game._$touchRight = jQuery('<a class="blockrain-touch blockrain-touch-right" />').appendTo(game._$gameholder);
      game._$touchRotateRight = jQuery('<a class="blockrain-touch blockrain-touch-rotate-right" />').appendTo(game._$gameholder);
      game._$touchRotateLeft = jQuery('<a class="blockrain-touch blockrain-touch-rotate-left" />').appendTo(game._$gameholder);
      game._$touchDrop = jQuery('<a class="blockrain-touch blockrain-touch-drop" />').appendTo(game._$gameholder);
    ...
    _randomShapes: function() {
      ...
      var shapeFuncs = [];
      if (true) { // normal
        jQuery.each(this._shapeFactory, function(k,v) { shapeFuncs.push(v); });
      } else { // dev test
        jQuery.each({square: this._shapeFactory.line}, function(k,v) { shapeFuncs.push(v); });
      }
      //console.log("shapeFuncs: ", shapeFuncs, "this._shapeFactory: ", this._shapeFactory);
    ...
    _setupControls: function(enable) {
      ...
      function keydown(evt) {
        var key = getKey(evt);
        jQuery.data(this, key, (jQuery.data(this, key) || 0) - 1);
        return handleKeyDown.call(this, evt);
      }

      function keyup(evt) {
        jQuery.data(this, getKey(evt), 0);
      ...
      jQuery(document) .unbind('keydown.blockrain')
                  .unbind('keyup.blockrain');

      if( ! game.options.autoplay ) {
        if( enable ) {
          jQuery(document) .bind('keydown.blockrain', keydown)
